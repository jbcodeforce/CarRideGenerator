...
```bash
carridegenerator$ sam build --use-container
```
...
```bash
carridegenerator$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
carridegenerator$ sam local start-api
carridegenerator$ curl http://localhost:3000/
```
...
```bash
carridegenerator$ sam logs -n HelloWorldFunction --stack-name carridegenerator --tail
```
...
```bash
carridegenerator$ pip install -r tests/requirements.txt --user
# unit test
carridegenerator$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
carridegenerator$ AWS_SAM_STACK_NAME="carridegenerator" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "carridegenerator"
```
